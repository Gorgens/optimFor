alternativaITC = function(
  ...
  while(hoje <= anoFim){
    ...
    if(rotacao == 1){
      idadecheck = idadeRot
      if(idadecheck == icR1){
        coeficiente = coeficiente + abs(ITCr1 - icR1)
      ...
      idadecheck = idadeRot
      if(idadecheck == icR2){
        coeficiente = coeficiente +  abs(ITCr2 - icR2)
  ...
  return(list(variavel = paste("T", talhao, "_", icR1, icR2, sep=""), coefFO = coeficiente, horizonteProd = round(producao, 2)))
}
